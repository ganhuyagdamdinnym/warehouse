...
// };
export const Totals = () => {
  const { isSuperAdmin } = useAuth(); // We use this boolean here
  const [data, setData] = useState<DashboardResponse | null>(null);
  const [loading, setLoading] = useState(true);
  ...
  const buildStats = (d: DashboardResponse) =>
    [
      ...
      { label: "Нийт зарлага", ...d.checkouts },
      { label: "Бараа материал", ...d.items },
      // Only show these if isSuperAdmin is true
      isSuperAdmin && { label: "Харилцагчид", ...d.contacts },
      isSuperAdmin && { label: "Агуулахууд", ...d.warehouses },
      { label: "Шилжүүлэг", ...d.transfers },
      { label: "Залруулга", ...d.adjustments },
      isSuperAdmin && { label: "Хэрэглэгчид", ...d.users },
    ].filter(Boolean) as (StatItem & { label: string })[]; // Removes the 'false' entries

  // Skeleton count should match the visible cards
  const skeletonCount = isSuperAdmin ? 8 : 5;

  return (
    <section
      className={`mb-8 grid grid-cols-2 ${isSuperAdmin ? "sm:grid-cols-4" : "sm:grid-cols-5"}  gap-3`}
    >
      {loading || !data
